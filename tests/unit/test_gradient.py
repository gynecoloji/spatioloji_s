...
class TestComputeGradientValidation:
    ...
    def test_all_missing_genes_raises(self, sp_gradient, interface_result):
        with pytest.raises(ValueError, match="No valid genes or features"):
            compute_gradient(sp_gradient, interface_result, genes=["nonexistent_gene"])

    def test_partial_missing_genes_warns(self, sp_gradient, interface_result):
        with pytest.warns(UserWarning, match="Skipping"):
            result = compute_gradient(
                sp_gradient, interface_result, genes=["gene_0", "nonexistent_gene"]
            )
        assert "gene_0" in result.gene_gradients.index
        assert "nonexistent_gene" not in result.gene_gradients.index

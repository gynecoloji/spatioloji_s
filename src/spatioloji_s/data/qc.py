...
class spatioloji_qc:
    ...
    def _plot_polygon_validity(self, results: pd.DataFrame, n_valid: int, n_invalid: int) -> None:
        ...
        if len(invalid_df):
            ...
            axes[1].barh(reason_counts.index, reason_counts.values, color="lightcoral", edgecolor="black")
            ...
            axes[1].text(
                0.5, 0.5, "No invalid polygons", ha="center", va="center", transform=axes[1].transAxes, fontsize=14
            )
            axes[1].set_title("Invalid Polygon Failure Reasons")
    ...
    * Transcript density (transcripts / um2) is computed and visualised.
    ...
    def _calculate_qc_metrics(self) -> None:
        ...
        * ``transcript_density`` -- transcript_counts / cell_area (transcripts / um2).
        ...
        if "cell_area" in self.sp.cell_meta.columns and "nucleus_area" in self.sp.cell_meta.columns:
            ...
            tc_col = (
                "transcript_counts" if "transcript_counts" in self.sp.cell_meta.columns else "transcript_counts_expr"
            )
            self.sp.cell_meta["transcript_density"] = self.sp.cell_meta[tc_col].values / safe_area
    ...
    def qc_cell_area(self, plot: bool = True) -> pd.DataFrame:
        ...
        if "cell_area" not in self.sp.cell_meta.columns:
            raise ValueError("'cell_area' not found in cell_meta. Was the object loaded with from_xenium()?")

        cell_area = self.sp.cell_meta["cell_area"].values.astype(float)
        nucleus_area = (
            self.sp.cell_meta["nucleus_area"].values.astype(float)
            if "nucleus_area" in self.sp.cell_meta.columns
            else np.full(self.sp.n_cells, np.nan)
        )
        ratio = (
            self.sp.cell_meta["nucleus_cell_ratio"].values.astype(float)
            if "nucleus_cell_ratio" in self.sp.cell_meta.columns
            else np.full(self.sp.n_cells, np.nan)
        )

        area_pass = (cell_area >= self.config.cell_area_min) & (cell_area <= self.config.cell_area_max)
        ...
        if plot:
            ...
            axes[0].axvline(
                self.config.cell_area_min, color="red", linestyle="--", label=f"min {self.config.cell_area_min} um2"
            )
            axes[0].axvline(
                self.config.cell_area_max, color="orange", linestyle="--", label=f"max {self.config.cell_area_max} um2"
            )
            ...
            if valid.any():
                ...
                axes[2].plot(
                    x_line,
                    x_line * self.config.nucleus_cell_ratio_max,
                    color="red",
                    linestyle="--",
    ...
    def qc_transcript_metrics(self, plot: bool = True) -> pd.DataFrame:
        ...
        tc_col = "transcript_counts" if "transcript_counts" in self.sp.cell_meta.columns else "transcript_counts_expr"
        ...
        print(
            f"  Cells with < {self.config.transcript_counts_min} transcripts: "
            f"{(tc < self.config.transcript_counts_min).sum():,}"
        )
        print(f"  Cells with < {self.config.n_features_min} features    : {(nf < self.config.n_features_min).sum():,}")
        ...
        if plot:
            ...
            axes[0].axvline(
                self.config.transcript_counts_min,
                color="red",
                linestyle="--",
            ...
            axes[1].axvline(
                self.config.n_features_min, color="red", linestyle="--", label=f"min = {self.config.n_features_min}"
            )
            ...
            sc = axes[2].scatter(
                ...
                cmap="Reds",
                vmin=0,
                vmax=self.config.pct_counts_neg_max * 100 * 2,
                s=2,
                alpha=0.4,
                linewidths=0,
            )
            axes[2].axvline(np.log1p(self.config.transcript_counts_min), color="red", linestyle="--", linewidth=1)
            axes[2].axhline(self.config.n_features_min, color="orange", linestyle="--", linewidth=1)
    ...
    def qc_control_metrics(self, plot: bool = True) -> pd.DataFrame:
        ...
        pct_cols = [
            f"pct_counts_{p}" for p in self.config.neg_prefixes if f"pct_counts_{p}" in self.sp.cell_meta.columns
        ]
        ...
        if plot and pct_cols:
            ...
            bars = axes[0].bar(labels, means_pct, color=bar_colors[: len(labels)], alpha=0.85, edgecolor="black")
            axes[0].axhline(
                self.config.pct_counts_neg_max * 100,
                color="red",
                linestyle="--",
            ...
            axes[1].axhline(10.0, color="red", linestyle=":", label="XOA error 10%", linewidth=1.5)
    ...
    def qc_nucleus_cell_ratio(self, plot: bool = True) -> pd.DataFrame:
        ...
        print(
            f"  Cells with ratio > {self.config.nucleus_cell_ratio_max} (seg. error): {n_err:,}  ({100 - pct_ok:.1f}%)"
        )
        ...
        if plot:
            ...
            r_valid = ratio[valid]
            axes[0].hist(r_valid[r_valid <= 2], bins=80, color="steelblue", alpha=0.75, edgecolor="none")
            axes[0].axvline(
                self.config.nucleus_cell_ratio_max,
                color="red",
                linestyle="--",
            ...
            plt.suptitle("Xenium QC -- Segmentation Quality (Nucleus : Cell Ratio)", fontsize=13)
            plt.tight_layout()
    ...
    def qc_transcript_density(self, plot: bool = True) -> pd.DataFrame:
        ...
        if "transcript_density" not in self.sp.cell_meta.columns:
            raise ValueError(
                "'transcript_density' not in cell_meta. Ensure cell_area is present (loaded via from_xenium)."
            )
        ...
        if plot:
            x = self.sp.cell_meta.get("x_centroid", pd.Series(np.full(self.sp.n_cells, np.nan))).values
            y = self.sp.cell_meta.get("y_centroid", pd.Series(np.full(self.sp.n_cells, np.nan))).values
            ...
            axes[0].hist(density[valid], bins=80, color="steelblue", alpha=0.75, edgecolor="none")
            axes[0].set_xlabel("Transcript density (tx / um2)")
            axes[0].set_ylabel("Cells")
            axes[0].set_title("Transcript Density Distribution")

            if has_coords:
                sc = axes[1].scatter(x[valid], y[valid], c=density[valid], cmap="viridis", s=1, alpha=0.6, linewidths=0)
                axes[1].invert_yaxis()
    ...
    def _plot_polygon_validity_xenium(
        ...
        if has_coords:
            ...
            axes[2].scatter(x[valid_mask], y[valid_mask], c="lightgreen", s=1, alpha=0.3, linewidths=0, label="Valid")
            if invalid_mask.any():
                axes[2].scatter(
                    x[invalid_mask], y[invalid_mask], c="red", s=4, alpha=0.8, linewidths=0, label="Invalid"
                )
            axes[2].invert_yaxis()
    ...
    def filter_cells(self, custom_filters: dict | None = None) -> pd.Series:
        ...
        * ``nucleus_cell_ratio`` <= ``nucleus_cell_ratio_max`` (if available)
        ...
        if "cell_area" in self.sp.cell_meta.columns:
            mask &= (self.sp.cell_meta["cell_area"] >= self.config.cell_area_min) & (
                self.sp.cell_meta["cell_area"] <= self.config.cell_area_max
            )

        if "nucleus_cell_ratio" in self.sp.cell_meta.columns:
            mask &= self.sp.cell_meta["nucleus_cell_ratio"].isna() | (
        ...
        reasons: dict[str, int] = {
            f"{tc_col} < {self.config.transcript_counts_min}": int(
                (self.sp.cell_meta[tc_col] < self.config.transcript_counts_min).sum()
            ),
            f"n_features < {self.config.n_features_min}": int(
                (self.sp.cell_meta["n_features_by_counts"] < self.config.n_features_min).sum()
            ),
            f"pct_neg > {self.config.pct_counts_neg_max:.0%}": int(
                (self.sp.cell_meta["pct_counts_neg"] > self.config.pct_counts_neg_max).sum()
            ),
            f"pct_mt > {self.config.pct_counts_mt_max:.0%}": int(
                (self.sp.cell_meta["pct_counts_mt"] > self.config.pct_counts_mt_max).sum()
            ),
        }
        if "cell_area" in self.sp.cell_meta.columns:
        ...
        if "nucleus_cell_ratio" in self.sp.cell_meta.columns:
            reasons[f"nucleus_cell_ratio > {self.config.nucleus_cell_ratio_max}"] = int(
                (self.sp.cell_meta["nucleus_cell_ratio"].fillna(0) > self.config.nucleus_cell_ratio_max).sum()
            )
        if "QC_polygon_valid" in self.sp.cell_meta.columns:
            reasons["invalid polygon geometry"] = int((~self.sp.cell_meta["QC_polygon_valid"]).sum())
    ...
    def filter_genes(self, plot: bool = True) -> pd.Series:
        ...
        if method == "percentile":
            baseline = (
                float(np.percentile(ctrl_totals, self.config.gene_percentile_threshold)) if ctrl_totals.size else 0.0
            )
            keep_panel = gene_totals > baseline
            desc = f"{self.config.gene_percentile_threshold}th pct of ctrl baseline ({baseline:.1f} counts)"
            ...
            raise ValueError(f"Unknown gene_filter_method '{method}'. Use 'percentile', 'absolute', or 'min_cells'.")
        ...
        if plot:
            ...
            axes[0].hist(
                np.log1p(gene_totals), bins=60, color="steelblue", alpha=0.75, edgecolor="none", label="All panel genes"
            )
            axes[0].hist(
                np.log1p(gene_totals[keep_panel]),
                bins=60,
                color="seagreen",
                alpha=0.65,
                edgecolor="none",
    ...
    def summarize_qc(self, plot: bool = True) -> dict:
        ...
        if "cell_filtering" in self.qc_metrics:
            cf = self.qc_metrics["cell_filtering"]
            print(f"\nCells  : {cf['n_before']:,} -> {cf['n_after']:,} ({cf['pct_kept']:.1f}% kept)")
        ...
        if plot and "cell_filtering" in self.qc_metrics and "gene_filtering" in self.qc_metrics:
            ...
            for ax, title, before, after in [
                ...
                bars = ax.bar(
                    ["Before QC", "After QC"], [before, after], color=["lightcoral", "lightgreen"], edgecolor="black"
                )
                for i, bar in enumerate(bars):
                    h = bar.get_height()
                    lbl = f"{h:,}\n({after / before * 100:.1f}%)" if i == 1 else f"{h:,}"
                    ax.text(bar.get_x() + bar.get_width() / 2, h, lbl, ha="center", va="bottom", fontsize=9)

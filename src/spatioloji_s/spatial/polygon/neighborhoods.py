...
def neighborhood_enrichment(sp: 'spatioloji',
    ...
    # Observed contacts
    print(f"  → Counting observed contacts...")
...
def niche_identification(sp: 'spatioloji',
    ...
    if store:
        ...
        print(f"  ✓ Stored in cell_meta['niche']")
    
    # Report
    counts = labels.value_counts(dropna=False)
    print(f"  ✓ Niche identification results:")
...
def boundary_cells(sp: 'spatioloji',
    ...
    if store:
        ...
        print(f"  ✓ Stored 'boundary_score' and 'is_boundary' in cell_meta")
    
    n_boundary = is_boundary.sum()
    n_total = (labels.notna() & (graph.get_degree() > 0).reindex(
        sp.cell_index, fill_value=False) if hasattr(graph.get_degree(), 'reindex') 
        else labels.notna()).sum()

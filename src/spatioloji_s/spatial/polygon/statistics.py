...
def contact_permutation_test(
    ...
    if correction == "bonferroni":
        ...
        adjusted[sorted_idx] = np.minimum.accumulate(adjusted[sorted_idx][::-1])[::-1]
    ...
    results["direction"] = np.where(results["z_score"] > 0, "enriched", "depleted")
...
def morphology_association_test(
    sp: spatioloji, metric: str, group_col: str | None = None, expression_gene: str | None = None, test: str = "kruskal"
) -> dict:
    """
    ...
    if group_col is not None:
        ...
        groups = [grp[morph_col].values for _, grp in data.groupby(group_col)]
    ...
    if expression_gene is not None:
        ...
        if sp.expression.is_sparse:
            expr_values = np.array(sp.expression._data[:, gene_idx].toarray()).flatten()
        else:
        ...
        combined = pd.DataFrame({"morph": morph_values, "expr": expr_series}).dropna()

        if len(combined) < 3:
            raise ValueError("Need at least 3 cells with both values")

        if test == "spearman":
            corr, pval = scipy_stats.spearmanr(combined["morph"], combined["expr"])
        elif test == "pearson":
            corr, pval = scipy_stats.pearsonr(combined["morph"], combined["expr"])
        else:
...
def spatial_autocorrelation_test(
    ...
    # Select metrics
    if metrics is None:
        metrics = sp.cell_meta.select_dtypes(include=[np.number]).columns.tolist()
    ...
    for metric in metrics:
        ...
        if len(vals) < 3 or vals.var() == 0:
            records.append(
                {"metric": metric, "statistic": np.nan, "expected": np.nan, "z_score": np.nan, "p_value": np.nan}
            )
            continue

        result = spatial_autocorrelation(sp, graph, metric, method=method, store=False)
    ...
    if valid_p.sum() > 0:
        ...
        if correction == "bonferroni":
            ...
            valid_adj[sorted_idx] = np.minimum.accumulate(valid_adj[sorted_idx][::-1])[::-1]
...
def boundary_enrichment_test(
    ...
    is_boundary = boundary_cells(sp, graph, group_col, min_different_fraction=min_different_fraction, store=False)
    ...
    for p in range(n_permutations):
        ...
        perm_boundary = rng.choice(labeled_indices, size=total_boundary, replace=False)
    ...
    for t in unique_types:
        ...
        # Two-sided p-value
        n_extreme = np.sum(np.abs(perm_vals - perm_mean) >= np.abs(n_boundary - perm_mean))

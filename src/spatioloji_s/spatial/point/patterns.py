...
def morans_i(
    ...
    if not local:
        ...
        pvalues = np.zeros(n)
        for c in range(n):
            extreme = np.sum(np.abs(perm_local[:, c] - perm_mean[c]) >= np.abs(local_i[c] - perm_mean[c]))
        ...
        print(
            f"  ✓ Local Moran's I / LISA ({label}{layer_str}): "
...
def getis_ord_gi(
    ...
    if not permutation_pvalue:
        ...
        pvalues = np.zeros(n)
        for c in range(n):
            extreme = np.sum(np.abs(perm_gi[:, c] - perm_mean[c]) >= np.abs(gi[c] - perm_mean[c]))
            pvalues[c] = (extreme + 1) / (n_permutations + 1)
...
def co_occurrence(
    ...
    if type_pairs is None:
        type_pairs = [(types[i], types[j]) for i in range(len(types)) for j in range(i, len(types))]
    ...
    for bin_idx in range(n_bins):
        ...
        pairs_in_bin = pairs_outer - tree.query_pairs(r_inner) if r_inner > 0 else pairs_outer

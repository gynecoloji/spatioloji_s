...
def compute_contrastive_scores(
    ...
    edge_lookup: dict[tuple, dict] = {
        (e['lr_name'], e['sender_type'], e['receiver_type']): e
        for e in edge_data
    }
    ...
    records: list[dict] = []
    n_rows = len(sig)
    print(f"\n[Layer3 Contrastive] {n_rows} combinations × "
          f"2 null models × {n_permutations} permutations")
    ...
    for row_idx, row in sig.iterrows():
        ...
        L    = _get_expr_vector(expr, gene2idx,
                                str(meta.get('ligand', '')).split('|'))
        R    = _get_expr_vector(expr, gene2idx,
                                str(meta.get('receptor', '')).split('|'))
        ...
        # Null_expr
        null_expr = _null_expression(
            L, R, ia_raw, ib_raw, w_raw, s_mask, r_mask, n_permutations, rng
        )

        # Null_geo
        null_geo = _null_geometry(
            L, R, ia_raw, ib_raw, w_raw, n_permutations, rng
        )
        ...
        contrastive_score = float(
            np.sqrt(max(z_expr, 0.0) * max(z_geo, 0.0))
        )
        ...
        if (row_idx + 1) % 20 == 0:
            print(f"  [{row_idx + 1}/{n_rows}] {lr_name} "
                  f"{sender_type}→{recv_type}: {driver_type}")

    if not records:
        return pd.DataFrame()

    df = pd.DataFrame(records).sort_values(
        'contrastive_score', ascending=False
...
def _polygon_laplacian(
    ...
    id2sp = {gid: sp_idx for sp_idx, gid in enumerate(cell_ids)
             if gid in graph_idx}
    ...
    A_sp = sparse.csr_matrix(
        (vals, (sp_rows, sp_cols)), shape=(n, n), dtype=np.float32
    )
...
def run_layer3(
    ...
    contrastive_df = compute_contrastive_scores(
        sp, layer1_results, layer2_results,
    ...
    X_flat, pair_list, lr_names = _build_communication_matrix(
        layer2_results, contrastive_df, n_cells=len(cell_ids)
    )
    print(f"  X_flat shape: {X_flat.shape}  "
          f"({len(pair_list)} contact pairs × {len(lr_names)} LR pairs)")
    ...
    programs_df = _interpret_programs(
        A, B, H, lr_names, cell_ids,
        type_masks, K, top_n=top_n_interpret,
    )

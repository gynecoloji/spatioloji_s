...
def _build_geometry_arrays(
    ...
    fb   = pd.Series(
        sp.cell_meta.get('free_boundary_fraction',
                         pd.Series(1.0, index=range(len(cell_ids)))).values,
        index=cell_ids,
    )
    sol  = pd.Series(
        sp.cell_meta.get('morph_solidity',
                         pd.Series(1.0, index=range(len(cell_ids)))).values,
        index=cell_ids,
    )
    ...
    return dict(
        ...
        ia=ia_arr, ib=ib_arr,
        fa=fa_arr, fb=fb_arr,
        dist=dist, d_norm=d_norm.astype(np.float32), decay=decay,
        fb_a=fb_a, fb_b=fb_b,
...
def _score_one_pair(
    ...
    T_vals, sender_ot, receiver_ot = _sinkhorn(
        a, b, ia_ot, ib_ot, cost, n, reg=ot_reg, n_iter=ot_iter
    )

    # ── Message Passing ───────────────────────────────────────────────────────
    ia_mp, ib_mp, w_mp = _geo_weight(geo, sig_type, s_mask, r_mask)
    m_vals, signal_mp, outgoing_mp = _message_passing(
        L, R, ia_mp, ib_mp, w_mp, n
    )
...
def run_layer2(
    ...
    for row_idx, row in sig.iterrows():
        ...
        result = _score_one_pair(
            pair, sig_type, sender_type, recv_type,
            L, R, geo, type_masks, cell_ids,
            ot_reg, ot_iter, hub_percentile,
        )
    ...
    if len(scores_df):
        print(f"  combined_strength range: "

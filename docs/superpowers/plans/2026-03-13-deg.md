...
# Populated incrementally as backends are implemented (Tasks 4–9).
# Finalized to the full four-method definition in Task 10.
_BACKEND_MAP: dict = {}
...
def _deseq2_backend(
    ...
    # counts_fg/counts_bg columns are ordered by gene_names (set above via counts_df),
    # so mean/pct columns are computed independently from the original ordering.
